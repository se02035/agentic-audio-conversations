"""Unit tests for the ``cancel_podcast`` MCP tool."""
...
async def test_cancel_podcast_unknown_job_id() -> None:
    ...
    async with Client(mcp) as client:
        with pytest.raises(ToolError, match="Unknown job_id"):
            await client.call_tool("cancel_podcast", {"job_id": "missing"})


async def test_cancel_podcast_is_noop_when_already_succeeded(sample_script_yaml: str) -> None:
...
async def test_cancel_podcast_stops_further_batches(sample_script_yaml: str) -> None:
    """cancel_podcast returns immediately; later mocked batches are skipped."""
    batches: list[int] = []
...
async def test_cancel_podcast_gcs_only_job_does_not_start_worker(
    ...
    async with Client(mcp) as client:
        ...
        cancelled = tool_data(await client.call_tool("cancel_podcast", {"job_id": job_id}))
    assert cancelled["status"] == JobStatus.cancelled
