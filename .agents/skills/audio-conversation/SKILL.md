...
Orchestrate conversation/narration audio with Google Cloud TTS Chirp 3 HD on `eu-texttospeech.googleapis.com`. Translation uses `translate-eu.googleapis.com`. Processing stays in the EU.
...
`start_conversation` must return before TTS finishes. It writes `status.json` as `queued` and checks EU `list_voices` first. Poll `get_conversation_status`. There is no download tool.
...
3. `.env` with `GOOGLE_CLOUD_PROJECT`. MCP also needs `AUDIO_CONVERSATION_GCS_STAGING_BUCKET` (EU bucket). CLI uploads are optional.
...
uv run tts-audio-conversation synthesize --script script_de.yaml --output output/episode.wav
# optional upload:
uv run tts-audio-conversation synthesize --script script_de.yaml --output output/episode.wav \
  --gcs-uri gs://your-eu-bucket/conversation/episode_$(date +%s).wav
uv run tts-audio-conversation download --gcs-uri gs://your-eu-bucket/conversation/episode.wav --output output/episode.wav
```
...
Anonymous Streamable HTTP at `http://127.0.0.1:8000/mcp`. `start_conversation` must return before TTS finishes. Poll `get_conversation_status`. There is no download tool.
...
Tools: `validate_script`, `start_conversation` (YAML/JSON string payload, optional `translate_to`), `get_conversation_status`, `cancel_conversation` (cooperative between batches). Send the script in the tool arguments, not a `gs://` link. Cap is `AUDIO_CONVERSATION_MAX_SCRIPT_BYTES` (default 512 KiB).

Cursor/VS Code MCP config: HTTP URL `http://127.0.0.1:8000/mcp`, no auth. Debug with launch config **Audio Conversation: MCP HTTP**.

On success, `audio_uri` is the WAV blob. Download it with the caller's GCS credentials. Multiple jobs run concurrently (`AUDIO_CONVERSATION_MAX_CONCURRENT_JOBS`, default 4).
...
- Stale `queued`/`running` after a crash: `get_conversation_status` fails the job after 30 min with no heartbeat. There is no resume — start a new job.
...
uv run tts-audio-conversation --help
```
